...
def preprocess(input_img: torch.Tensor) -> torch.Tensor:
    return input_img * 255.0


def postprocess(output_img: torch.Tensor) -> torch.Tensor:
    return output_img / 255.0
...
class RLFN_Prune(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, feature_channels=46, mid_channels=48, upscale=4):
        ...
        self.conv_1 = block.conv_layer(in_channels, feature_channels, kernel_size=3)
